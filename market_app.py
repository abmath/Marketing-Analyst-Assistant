...
def main():
    ...
    # ── Table selection (shared across all tabs) ───────────────────────────────
    st.markdown('<div class="section-label">Select a Table</div>', unsafe_allow_html=True)
    ...
    # Reset query/insight state when the user switches tables
    if st.session_state.get("_last_selected_table") != selected:
        st.session_state.pop("query_result", None)
        st.session_state.pop("insights", None)
        st.session_state["_last_selected_table"] = selected

    st.divider()

    # ── Tabs ───────────────────────────────────────────────────────────────────
    tab_preview, tab_query, tab_slides = st.tabs(
        ["📋 Data Preview", "💬 NL Query", "✨ Slide Suggestions"]
    )
    ...
    with tab_query:
        st.caption(
            f"Querying **{selected}** — {len(df):,} rows × {len(df.columns)} columns. "
            "Switch tables from the selector above."
        )
        ...
        with right:
            ...
            run_clicked = col_run.button(
                "▶ Run Query", type="primary", disabled=not ollama_ok, key="run_query_btn"
            )
            if col_clear.button("Clear", key="clear_query_btn"):
        ...
        # Render the most recent query result (persists across reruns)
        if "query_result" in st.session_state:
            qr = st.session_state["query_result"]
            result, attempts, error = qr["result"], qr["attempts"], qr["error"]

            with st.expander(
                f"🛠 Generated code "
                f"{'(solved in 1 attempt)' if len(attempts) == 1 and not error else f'({len(attempts)} attempt(s))'}",
                expanded=False,
            ):
            ...
            if error:
                ...
                st.caption(
                    "Tip: rephrase using column names visible in the Data Preview tab, "
                ...
                if isinstance(result, pd.DataFrame):
                    ...
                    st.download_button(
                        ...
                        file_name=f"query_result_{re.sub(r'[^\w]', '_', selected)[:40]}.csv",
                        mime="text/csv",
        ...
        if not ollama_ok:
            st.info("Start Ollama to enable querying: `ollama serve` then `ollama pull phi3`")

    # ── Tab 3: PPT Insights ────────────────────────────────────────────────────
    with tab_slides:
        st.caption(
            f"Phi-3 will analyze **{selected}** and draft 5 slide ideas for your marketing review deck."
        )
